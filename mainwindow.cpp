...
//  for (int i=0;i<10;i++) pt[i] = NULL;
  ...
  ui->customPlot->setInteractions(QCP::iRangeDrag | QCP::iRangeZoom | QCP::iSelectAxes |
                                  QCP::iSelectLegend); // | QCP::iSelectItems | QCP::iSelectPlottables);
  ...
  connect(ui->customPlot, SIGNAL(itemClick(QCPAbstractItem*,QMouseEvent*)), this, SLOT(itemClicked(QCPAbstractItem*,QMouseEvent*)));
  ...
  myTcpClient->show();
  connect(myTcpClient, SIGNAL(readyMsgTcp(QVector<double> *, QVector<double> *, QVector<double> *)), this, SLOT(addPoints(QVector<double> *, QVector<double> *, QVector<double> *)));
}
...
void MainWindow::addPoints(QVector<double> *VectTime, QVector<double> *vect, QVector<double> *vectX){
    qDebug()<<"addPoints()";
    removeAllGraphs();

    ui->customPlot->addGraph();
    ui->customPlot->graph()->setName(QString("New graph %1").arg(ui->customPlot->graphCount()-1));
    ui->customPlot->graph()->setData(VectTime[0], vect[0]);
    ui->customPlot->graph()->setLineStyle(QCPGraph::lsLine);
...
void MainWindow::graphClicked(QCPAbstractPlottable *plottable, int dataIndex)
{
  ...
  if ( pt[pt_ptr] )
      delete  pt[pt_ptr];
  pt[pt_ptr++] = new point_tag(plottable, dataKey, dataValue);
  if (pt_ptr >= 10) pt_ptr = 0;
...
void MainWindow::legendSelectChanged(QCPLegend::SelectableParts parts)
{
    moveLegend();
}
